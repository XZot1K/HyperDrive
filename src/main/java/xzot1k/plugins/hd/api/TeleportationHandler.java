...
public class TeleportationHandler implements Runnable {
    ...
    @Override
    public void run() {
        for (UUID playerUniqueId : new ArrayList<>(getTeleportTempMap().keySet())) {
                            ...
                            case "warp":
                                if (teleportTemp.getTeleportValue() != null) {
                                    ...
                                    if (warp != null && warp.getWarpLocation() != null) {
                                        ...
                                        if (getPluginInstance().getHookChecker().isNotSafe(player, warpLocation, HookChecker.CheckType.WARP)) {
                                            getAnimation().stopActiveAnimation(player);
                                            getTeleportTempMap().remove(playerUniqueId);
    ...
    public boolean isTeleporting(Player player) {
        RandomTeleportation randomTeleportation = RandomTeleportation.getRTPInstance(player.getUniqueId());
        return (!getTeleportTempMap().isEmpty() && getTeleportTempMap().containsKey(player.getUniqueId())) || (randomTeleportation != null);
    }
        ...
        if (player.getVehicle() != null && teleportVehicle) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    try {
                        Method method = entity.getClass().getDeclaredMethod("addPassenger", Entity.class);
                        method.invoke(entity, player);
    ...
    public void randomlyTeleportPlayer(Player player, World world) {
        ...
        RandomTeleportation.getRtpMap().put(player.getUniqueId(), new RandomTeleportation(player, world, false));
    }

    public void updateDestinationWithRandomLocation(Player player, World world) {
        RandomTeleportation.getRtpMap().put(player.getUniqueId(), new RandomTeleportation(player, world, true));
    }
