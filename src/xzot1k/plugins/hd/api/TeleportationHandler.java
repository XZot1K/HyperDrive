...
public class TeleportationHandler implements Runnable
{
    ...
    private String teleportTitle, teleportSound, teleportSubTitle, delayTitle, delaySubTitle;
    ...
    public TeleportationHandler(HyperDrive pluginInstance)
    {
        ...
        setTeleportTitle(getPluginInstance().getConfig().getString("teleportation-section.teleport-title"));
        setTeleportSubTitle(getPluginInstance().getConfig().getString("teleportation-section.teleport-sub-title"));
        setDelayTitle(getPluginInstance().getConfig().getString("teleportation-section.delay-title"));
        setDelaySubTitle(getPluginInstance().getConfig().getString("teleportation-section.delay-sub-title"));
                ...
                if (teleportTemp != null)
                {
                    ...
                    if (teleportTemp.getSeconds() > 0)
                    {
                        ...
                        if (warp != null && teleportTemp.getSeconds() <= 5 && teleportTemp.getSeconds() > 0)
                        {
                            ...
                            {
                                if (getDelayTitle() != null && getDelaySubTitle() != null)
                                    getPluginInstance().getManager().sendTitle(player, getDelayTitle().replace("{warp}", warp.getWarpName())
                                            .replace("{duration}", String.valueOf(teleportTemp.getSeconds())), getDelaySubTitle().replace("{warp}", warp.getWarpName())
                        ...
                        if (teleportTemp.getTeleportTypeId().equalsIgnoreCase("warp"))
                        {
                            if (teleportTemp.getTeleportValue() != null)
                            {
                                if (warp != null)
                                {
                                    ...
                                    {
                                        getPluginInstance().getManager().sendActionBar(player, Objects.requireNonNull(getPluginInstance()
                                                .getConfig().getString("language-section.teleport-fail-message"))
                                                .replace("{warp}", warp.getWarpName()));
                                    ...
                                    if (!getTeleportSound().equalsIgnoreCase(""))
                                        player.getWorld().playSound(player.getLocation(), Sound.valueOf(getTeleportSound()), 1, 1);

                                    if (getTeleportTitle() != null && getTeleportSubTitle() != null)
                                        getPluginInstance().getManager().sendTitle(player, getTeleportTitle().replace("{warp}", warp.getWarpName()),
                                                getTeleportSubTitle().replace("{warp}", warp.getWarpName()), 0, 5, 0);

                                    getPluginInstance().getManager().sendActionBar(player, Objects.requireNonNull(getPluginInstance().getConfig().getString("teleportation-section.teleport-bar-message"))
                                            .replace("{warp}", warp.getWarpName()));

                                    getPluginInstance().getManager().sendCustomMessage(Objects.requireNonNull(getPluginInstance().getConfig().getString("language-section.teleportation-engaged"))
                                            .replace("{warp}", warp.getWarpName()).replace("{duration}", String.valueOf(teleportTemp.getSeconds())), player);
                                }
                            }
    ...
    {
        return (!getTeleportTempMap().isEmpty() && getTeleportTempMap().containsKey(player.getUniqueId())) || getRandomTeleportingPlayers().contains(player.getUniqueId());
    }
        ...
        new BukkitRunnable()
        {
            ...
            public void run()
            {
                ...
                if (maxTries > -1 ? (!foundSafeLocation && tries <= maxTries) : !foundSafeLocation)
                {
                    ...
                    if (finalBasedLocation.getWorld() == null) return;
                    ...
                    if (!isLocationHookSafe(player, foundBlock.getLocation())) return;
                    ...
                    for (int i = -1; ++i < forbiddenMaterialList.size(); )
                    {
                        ...
                        if (materialLine != null && materialLine.contains(":"))
                        {
                            ...
                            Material material = Material.getMaterial(materialArgs[0].toUpperCase().replace(" ", "_").replace("-", "_"));
                            int durability = 0;

                            if (getPluginInstance().getManager().isNumeric(materialArgs[1]))
                                durability = Integer.parseInt(materialArgs[1]);

                            if (material != null && foundBlock.getType() == material && (durability == -1 || foundBlock.getData() == durability))
                            {
                                isForbidden = true;
                                break;
                            }
                    ...
                    if (isForbidden && foundBlock.getType() == Material.AIR)
                    {
                        ...
                        for (int i = -1; ++i < forbiddenMaterialList.size(); )
                        {
                            ...
                            if (materialLine != null && materialLine.contains(":"))
                            {
                                ...
                                if (material != null && blockUnder.getType() == material && (durability == -1 || blockUnder.getData() == durability))
                                {
                        ...
                        if (!underIsForbidden) isForbidden = false;
                    }
                    ...
                    if (!isForbidden)
                    {
                        ...
                        RandomTeleportEvent randomTeleportEvent = new RandomTeleportEvent(getPluginInstance(), newLocation, player);
                        ...
                        if (animationLine != null && animationLine.contains(":"))
                        {
                            ...
                            if (animationArgs.length >= 2)
                            {
                                ...
                                getAnimation().playAnimation(player, animationArgs[1].toUpperCase().replace(" ", "_").replace("-", "_"), animation, 1);
                            }
                        }

                        if (!getTeleportSound().equalsIgnoreCase(""))
                            Objects.requireNonNull(newLocation.getWorld()).playSound(newLocation, Sound.valueOf(getTeleportSound()), 1, 1);


                        getPluginInstance().getManager().sendCustomMessage(Objects.requireNonNull(getPluginInstance().getConfig().getString("language-section.random-teleported"))
                                .replace("{tries}", String.valueOf(tries - 1)).replace("{x}", String.valueOf(newLocation.getBlockX()))
                                .replace("{y}", String.valueOf(newLocation.getBlockY())).replace("{z}", String.valueOf(newLocation.getBlockZ()))
    ...
    {
        if (world.getEnvironment() == World.Environment.NORMAL) return world.getHighestBlockYAt((int) x, (int) z);
    ...
    public boolean isLocationHookSafe(Player player, Location location)
    {
        ...
        if (worldGuardPlugin != null && getPluginInstance().getConfig().getBoolean("hooks-section.use-worldguard"))
        {
            ...
            {
                if (!WG_6.passedWorldGuardHook(location)) isSafeLocation = false;
            } else if (worldGuardPlugin.getDescription().getVersion().toLowerCase().startsWith("7"))
            {
                if (!WG_7.passedWorldGuardHook(location)) isSafeLocation = false;
            }
        }
        ...
        if (griefPreventionPlugin != null && getPluginInstance().getConfig().getBoolean("hooks-section.use-grief-prevention"))
        {
            Claim claimAtLocation = GriefPrevention.instance.dataStore.getClaimAt(location, false, null);
            if (claimAtLocation != null) isSafeLocation = false;
        }
    ...
    private String getTeleportTitle()
    {
        return teleportTitle;
    }

    private void setTeleportTitle(String teleportTitle)
    {
        this.teleportTitle = teleportTitle;
    }

    private String getTeleportSubTitle()
    {
        return teleportSubTitle;
    }

    private void setTeleportSubTitle(String teleportSubTitle)
    {
        this.teleportSubTitle = teleportSubTitle;
    }

    private String getDelayTitle()
    {
        return delayTitle;
    }

    private void setDelayTitle(String delayTitle)
    {
        this.delayTitle = delayTitle;
    }

    private String getDelaySubTitle()
    {
        return delaySubTitle;
    }

    private void setDelaySubTitle(String delaySubTitle)
    {
        this.delaySubTitle = delaySubTitle;
    }
    ...
    private String getTeleportSound() { return teleportSound; }
